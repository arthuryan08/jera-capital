...
## Tech Stack
- **Frontend**: Next.js 15 (App Router), TypeScript, Tailwind CSS, shadcn/ui, shadcn charts (Recharts)
...
- Use Zod for ALL external data validation (API inputs, form data, env vars)
- Write tests BEFORE or alongside implementation (TDD for engine functions)
- Use conventional commits: `feat:`, `fix:`, `test:`, `docs:`, `refactor:`, `chore:`
- Keep commits small and focused -- one logical change per commit

### When creating files
- Follow the folder structure above exactly
- Engine functions must be pure -- no imports from db/, modules/, or external services
- Components in `ui/` are auto-generated by shadcn -- never edit them manually
- Every new API endpoint needs a corresponding Zod schema

### When writing tests
- Engine tests: test edge cases (0 values, boundary days for tax brackets, large periods)
- Integration tests: use `buildApp()` + `app.inject()` with in-memory SQLite
- Frontend tests: schema validation, format utilities, component smoke tests
...
### Formatting & Style
- Brazilian Portuguese for UI labels and user-facing text
- English for code (variables, functions, comments, commit messages)
...
### Dependencies
**Backend**: fastify, @fastify/swagger, @fastify/swagger-ui, @fastify/cors, @fastify/jwt, fastify-type-provider-zod, zod, drizzle-orm, @libsql/client, uuid, bcryptjs
**Backend dev**: drizzle-kit, typescript, vitest, @types/bcryptjs, tsx
**Frontend**: next-auth@beta, @hookform/resolvers, react-hook-form, zod, recharts, sonner
**Frontend dev**: vitest, @testing-library/react, @testing-library/jest-dom, @vitejs/plugin-react, jsdom
